...
def generate_covers(
    ...
    # Pre-check: tem creditos suficientes pra TENTAR o lote inteiro?
    initial_state = credits_service.get_remaining(user_id)
    if initial_state.get("remaining", 0) < lote:
        return {
            "ok": False,
            "generated_ids": [],
            "credits_consumed": 0,
            ...
            "errors": [
                f"sem creditos suficientes (precisa {lote}, "
    ...
    for i in range(lote):
        ...
        # h. SO AGORA cobra o credito (capa ja esta entregue)
        ...
        logger.info(
            "cover worker: capa gerada user=%s cover_id=%s hash=%s cost=%.4f",
            user_id, cover_id, image_hash[:8], total_cost,
        )
