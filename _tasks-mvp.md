...
- **Criterio de pronto:** Smoke test (sem API) verde: 5 seeds do mesmo brief dao combinacoes distintas de cenario+pose+framing, sem_pessoa zera a pose, injecao no user_prompt OK. **VALIDACAO VISUAL PENDENTE em prod** (sem ANTHROPIC_API_KEY local): gerar 5 variacoes de "mulher sad underground" e confirmar (a) cenarios variados, (b) sem barras pretas, (c) Claude obedece o cenario em vez de copiar a cama. **Passo B (anti-repeticao de objetos via bloco machine-readable + recent_seeds) ADIADO** ate validar o Passo A visualmente.
