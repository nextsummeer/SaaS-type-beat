...
**Criado:** 2026-04-25
**Atualizado:** 2026-05-19 (noite — polish + fluxo de rascunhos)
...
**Iniciado:** 2026-04-25
**Status:** em-execucao
**Proximo passo:** Polish visual + fluxo de rascunhos pendentes (2026-05-19, noite +2) — segunda leva de design depois do deploy inicial Editorial Mono: (1) contraste dos cards subiu (bg-surface #0A0A0C→#0E0E12, borders 6%→9%) pra eles destacarem do fundo, (2) hover dos cards ganhou tint lavanda sutil (`--border-hover: rgba(199,181,255,0.30)` + glow `--glow-hover`), (3) banner da dashboard ganhou ilustracao nova (constelacao neural SVG com nos pulsantes e linhas conectando em gradient roxo→magenta) no lugar do waveform branco, (4) filtros de /beats redesenhados estilo Linear/Notion (pills tematizadas com icone + cor + glow sutil por status), (5) novo fluxo de rascunhos: `RascunhosPendentesList` na /agenda (sempre visivel, com empty state amigavel), drag de rascunho redireciona pra `/review?agendar_em=<ISO>` em vez de PATCH direto (forca revisao antes de publicar), `ProximasPublicacoesWidget` mostra rascunhos pendentes (3 estados: empty, callout proativo so com rascunho, footer sutil quando tem mix), bug fix em `DashboardStats` ("Em fila" agora conta rascunhos como o label promete). Build 17/17 paginas verde. Bloqueadores criticos seguem ativos: (1) Rary republicar beat `ee96c64f` pra validar fix codec, (2) aumento de quota YouTube, (3) OAuth verification, (4) decisao final billing. Bloco capa IA segue PAUSADO.
