...
**Criado:** 2026-04-25
**Atualizado:** 2026-06-02 (sessao 10: T4.45 corrige o furo do prompt caching no usage_tracker -- custo da capa agora soma os baldes de cache do Claude; 7 testes, suite 64 verde. Sessao tambem ALINHOU sem codigo arquitetura da landing + admin dashboard + mapa de custo real. ANTES sessao 9: T2.15 aceitar WAV -> converter pra MP3 320k no servidor ENTREGUE; tasks T2.15 + T4.44 criadas pras 2 features de upload pedidas pelo Gustavo. ANTES -- sessao 8: T4.40 capa IA -- letterbox removido + variacao pose/framing/cenario autoritativo; sessao 7 FECHADA: T4.37+T4.38+T4.39 entregues + rodada grande de polish de UI pos-T4.39 -- AudioPlayer multicolor+drag, modal capas React Portal, larguras padronizadas, abas /capas reorganizadas, botao salvar link inline na review)
...
#### `[!]` T2.15 — Aceitar WAV no upload + converter pra MP3 320kbps no servidor (ADIADA pro pos-MVP)
...
- **ADIADA pro pos-MVP (2026-06-02) — e REVERTIDA do codigo.** Foi implementada (Opcao B, servidor) e dava deploy, MAS na hora de subir o limite do bucket pra aceitar WAV grande descobrimos o bloqueador real: **os WAVs do Gustavo chegam a ~60MB e o plano Free do Supabase tem teto fixo de 50MB por arquivo** (config de bucket NAO passa disso; so o plano Pro $25/mes levanta). Decisao do Gustavo: **adiar WAV** e seguir **so com MP3** por enquanto -- a maioria dos produtores ja exporta MP3 pro YouTube, entao a friccao real e baixa. Codigo da T2.15 revertido (`git checkout` dos 4 arquivos pro estado pre-commit `c5e0388`): volta a aceitar so MP3, sem `transcode_to_mp3`, convert.py noop original.
- **Quando retomar, 2 caminhos (avaliados):** (A) **converter no NAVEGADOR** -- decodifica + encoda MP3 client-side (ja temos decode via essentia.js; falta um encoder tipo lamejs), sobe so o MP3 (~10MB), **dribla o teto de 50MB sem pagar Pro** -- HOJE e o caminho mais forte dado WAV de 60MB; (B) **plano Pro + manter servidor** (Opcao B original) -- levanta o teto mas paga $25/mes e sobe 60MB lento. Recomendacao: (A).
...
#### `[x]` T4.44 — Gerar capa por IA direto no /upload (modal: escolher brief salvo ou criar novo)
...
- **FEITO (2026-06-02):** descoberta-chave -- `POST /covers/generate` e SINCRONO (espera ~30s e ja retorna `generated_ids` prontos), entao NAO precisou de Realtime/polling no upload. Novo `lib/api.generateCover` (helper do endpoint, force_variation on). Novo `InlineCoverGenerator.tsx`: modal com 4 views (loading/picking/generating/done/error) -- escolhe brief salvo OU cria novo via `CapasWizard` (brief novo FICA SALVO via createBrief), botao "Gerar · 1 credito", orb signature da /capas no estado generating, preview da capa pronta + "Usar essa capa". `CoverPicker.tsx`: botao "Gerar capa com IA" na linha das tabs + empty state vira convite pra gerar inline; `handleAIGenerated` recarrega a biblioteca, seleciona a capa nova e abre Biblioteca>IA. Zero mudanca no backend. typecheck + build verdes (lint do projeto ja vinha com 25 erros pre-existentes em outros arquivos; meus arquivos limpos). **Validacao em prod pendente:** gerar 1 capa pelo /upload e ver ela entrar selecionada (consome 1 credito real).

#### `[x]` T4.45 — Custo do Claude com prompt caching no usage_tracker (somar baldes de cache)

- **Motivacao:** Gustavo pediu o custo REAL por geracao de capa (sem estimativa) pra fechar unit economics. Investigando o `usage_tracker` achei um furo: o Claude do `cover_prompt_builder` usa prompt caching ephemeral (system prompt de ~4k tokens com `cache_control`), e o builder ja CAPTURAVA `cache_read_input_tokens`/`cache_creation_input_tokens` -- mas so jogava no `metadata`. O `_calculate_cost` usava so `input_tokens`+`output_tokens` e IGNORAVA os baldes de cache. Resultado: custo do Claude da capa subestimado (sumia o custo de gravar o system prompt no cache na 1a geracao do lote -- ~$0.0204 real virava ~$0.0054 registrado).
- **Decisao (2026-06-02):** somar os 3 baldes de input no `_calculate_cost` sem dupla contagem (a Anthropic devolve cache_read/cache_creation em campos separados, ja fora de `input_tokens`): input normal + cache write (1.25x) + cache read (0.1x) + output. Multiplicadores como constantes (`CACHE_WRITE_MULT`/`CACHE_READ_MULT`). Corrige daqui pra frente; historico fica pra recalculo retroativo OPCIONAL (tokens de cache preservados no metadata das rows antigas).
- **Arquivos:**
  - `api/app/services/usage_tracker.py` (`CACHE_WRITE_MULT`/`CACHE_READ_MULT` + cache no `_calculate_cost` + params `cache_read_tokens`/`cache_write_tokens` no `track()`)
  - `api/app/services/cover_prompt_builder/builder.py` (passa cache_read/cache_write pro `track()`, nao so pro metadata)
  - `api/tests/services/test_usage_cost.py` (novo -- 7 testes do calculo com/sem cache)
- **Criterio de pronto:** `_calculate_cost` soma cache corretamente (1a capa fria ~$0.0204, quentes ~$0.0066); custo do Claude da capa para de subestimar; suite backend verde sem quebrar nada. **FEITO** -- 7 testes novos verdes, suite 64 passed. **Pendente (opcional, nao bloqueia):** recalculo retroativo do historico via metadata + reconciliacao do SUM(cost_usd) com a fatura real da Anthropic.
- **Dependencia:** T4.43 (mesmo usage_tracker), regra P5 (usage_tracker)
...
- **2026-06-02 (sessao 10 -- landing/admin alinhados + T4.45 custo do cache do Claude)** -- Sessao mista: discussao de arquitetura (sem codigo) + 1 correcao entregue. (1) **Landing page:** Gustavo quer 2 etapas (captura de leads pro beta + venda completa). Alinhado: fica DENTRO do `web/` como route group `(marketing)`, NAO projeto separado (reaproveita design Editorial Mono, 1 deploy/dominio). Captura = subset enxuto da venda, nao descartavel. Espera o NOME da marca (meta dele: decidir essa semana). Ordem certa: nome -> mensagem/posicionamento -> codigo. Memoria [[project_landing_arquitetura]]. (2) **Admin dashboard:** quer aba pra ele+Rary verem uso/custos de todos. Alinhado: rota `/admin` web no mesmo `web/` (descartei app desktop -- "desktop = mais seguro" e falso, seguranca vem do backend; custo de build/cert nao compensa pra 2 users). Acesso via `is_admin` dedicado (NAO o tier `internal`, que beta testers tambem tem) + backend FastAPI/service_role bypassa RLS apos checar admin + 2FA. Dados ja existem (api_usage/beats/posts). Comecar enxuto: validar metricas com SQL antes da UI. Memoria [[project_admin_dashboard_arquitetura]]. (3) **Custo real:** expliquei que cost_usd = tokens REAIS x tabela PRICING (nao fixo). Verifiquei: Gemini/Claude-texto/fal reais e corretos (fal usa tokens da resposta, T4.43). Achei o furo do cache -> virou **T4.45**. Tambem registrei que api_usage cobre so IA, nao infra (Supabase/Railway-ffmpeg). Memoria [[project_custo_tracking_real]]. Nao consegui puxar numero real do banco (MCP Supabase nao carregou nesta sessao + sem .env local); dei o custo decomposto calculado (capa: 1a ~$0.031 fria / ~$0.018 quente, sendo fal ~$0.011 fixo + Claude variavel pelo cache). (4) **T4.45 ENTREGUE:** corrigido o furo -- `_calculate_cost` agora soma input + cache_write(1.25x) + cache_read(0.1x) + output (Anthropic devolve cache em campos separados, sem dupla contagem); `track()` ganhou params cache_read_tokens/cache_write_tokens; builder.py passa os cache tokens pro track (antes so pro metadata). 7 testes novos em test_usage_cost.py, suite backend 64 passed (eram 57). Recalculo retroativo do historico fica opcional (tokens preservados no metadata).

- **2026-06-02 (sessao 9 -- setup de ferramentas + T2.15 WAV->MP3)** -- Sessao mista. (1) **Setup:** instalado Supabase MCP (read-only, travado no projeto) + Playwright MCP no Claude Code, ambos custo zero; `claude` adicionado ao PATH do Windows (`~/.local/bin`). MCPs precisam de conversa NOVA pra carregar as tools. Memoria [[project_supabase_mcp_instalado]] criada (token Supabase expira ~2026-07-01). (2) **2 features de upload pedidas pelo Gustavo, viraram tasks T2.15 + T4.44.** (3) **T2.15 implementada, depois ADIADA + REVERTIDA.** Cheguei a entregar (Opcao B servidor: `ffmpeg_service.transcode_to_mp3` + convert.py convertendo nao-MP3 + frontend aceitando WAV; commit `c5e0388` deployado). Mas ao ir mexer no limite do bucket pra WAV grande, achamos o bloqueador: **WAVs do Gustavo ~60MB > teto FIXO de 50MB/arquivo do Supabase Free** (config de bucket nao passa disso; so Pro $25/mes). Levantei o ponto cego: a Opcao B (subir WAV inteiro) bate nesse teto; a Opcao A (converter no navegador, subir so o MP3 ~10MB) driblaria sem pagar Pro. Gustavo decidiu **adiar WAV e seguir so com MP3** -- maioria dos produtores ja exporta MP3 pro YouTube, friccao real baixa. Codigo revertido via `git checkout 8a5eb0c` dos 4 arquivos (volta a MP3-only). (4) **T4.44 ENTREGUE:** gerar capa por IA direto no /upload. Descoberta: `/covers/generate` e sincrono (retorna ids prontos ~30s) -> sem polling. Novo `InlineCoverGenerator.tsx` (modal: escolhe brief salvo OU cria novo via CapasWizard que FICA SALVO, gera com orb signature, mostra preview, "Usar essa capa"). `lib/api.generateCover` + botao "Gerar capa com IA" no CoverPicker + empty state vira convite inline + auto-selecao pos-geracao. Zero backend. typecheck+build verdes (lint pre-existente com 25 erros noutros arquivos, meus limpos). Validacao em prod pendente (gerar capa real, consome 1 credito).
