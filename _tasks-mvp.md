...
- **2026-05-21 (sessao 3 cont.)** — T4.19 concluida. Pacote `api/app/services/cover_prompt_builder/` criado: `types.py` (CoverBrief/VariationAxes/BuildResult com Literal types pros slugs do brief v2), `system_prompt.py` (SYSTEM_PROMPT v2 integral em ingles, 9031 chars, marcado como receita secreta), `vocabulary.py` minimo (11 generos + 5 quem + 6 mood + 8 cenarios + 6 luzes, sem matriz de compatibilidade rica — fica pra proxima sessao). Decisao de implementacao: arquivo antigo `cover_prompt_builder.py` movido pra `legacy.py` dentro do pacote (Python nao permite arquivo e pasta com mesmo nome), `__init__.py` re-exporta `build_cover_prompt` do legacy ate T4.21 entregar o builder v2 — preserva pipeline em producao sem deploy parcial quebrado. 49 testes verdes (`pytest -m "not slow"`).
