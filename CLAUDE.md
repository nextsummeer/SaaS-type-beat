...
6. **Capa pode ser gerada por IA (aba dedicada `/capas`) ou enviada manualmente.** Geracao usa SYSTEM_PROMPT v2 estruturado (principio "Captured Not Composed" + DNA universal + anti-aesthetics + gramatica 7 blocos) + brief de 6+2 campos (genero primario+secundario, artista primario+secundario, quem aparece, mood, cenario, atmosfera de luz, nota livre opcional) + sorteio de 7 eixos de variacao em Python (subject_framing, camera_angle, time_of_day, sub_location, secondary_prop, motion_state, film_quirk). Claude Sonnet 4.6 (com prompt caching ephemeral no system block) monta prompt final → fal.ai gpt-image-2 quality=low gera imagem ($0.013/capa total, ~30s). NUNCA mencionar nome de artista real no prompt final (likeness via 3 camadas: nome direto + apelidos blocklist + frases-ancora genericas). Nota livre passa por sanitizer (anti prompt-injection + termos incompativeis com estetica). Capas ficam em biblioteca reusavel (`cover_library`) por produtor; coluna `variation_seeds JSONB` persiste os 7 eixos por capa. UX assincrona. Creditos por tier (Free 5 / Intermediario 15 / Premium 40). Upload manual nao consome creditos. Toda geracao registra em `api_usage` (incluindo cache_read/cache_write tokens). Detalhes em `docs/decisoes/2026-05-21-prompt-dna-capa-v2.md` (complementa `2026-05-21-geracao-de-capa-prompt-base-claude.md`).
