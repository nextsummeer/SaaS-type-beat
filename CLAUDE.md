...
- Vercel project: saa-s-type-beat.vercel.app (conta Gustavo, Hobby)
- Railway project: saas-type-beat-production.up.railway.app (porta 8080, root = /api)
...
6. **Capa pode ser gerada por IA (aba dedicada `/capas`) ou enviada manualmente.** Geracao usa SYSTEM_PROMPT v3 estruturado em 12 elementos: camera DNA video-still FIXA em 2 variantes (padrao calibrado + underground agressivo c/ termos "Heavy/blocky pixelation/vertical glitch line" liberados SO no underground) + thematic sentence + frase mestre + sujeito etnia-inline + frase-soco + wardrobe pool + sub-location sorteada do universo do artista + lighting + guard-rail anti-destruicao + color palette + optional details + energy + shot on fixo + references culturais + 3-word closer. Brief de 5+2 campos (genero primario+secundario, artista primario+secundario, quem aparece, mood, atmosfera de luz, nota livre opcional -- SEM cenario, inferido do universo do artista). Sub-locations curadas por artista no `artist_universe.py` (5 validados visualmente: Drake/Weeknd/Fakemink/Nettspend/Travis; expansivel; artistas fora do dict caem em fallback gracioso por genero). Anti-repeticao via query nas ultimas 5 variation_seeds do user quando botao "Gerar variacao". Palavras banidas (music video, cinematic, B-roll, director...) + references banidas (Drive, Wong Kar-wai, Vogue, Helmut Newton) vs permitidas (Larry Clark, Nan Goldin, Cobrasnake, Hedi Slimane). Anti-bias inline (sem asiaticos default, sem gang signs em crews). Claude Sonnet 4.6 com prompt caching ephemeral → fal.ai gpt-image-2 quality=low ($0.013/capa, ~30s). NUNCA mencionar nome do artista no prompt final (likeness via 3 camadas: nome direto + apelidos blocklist + frases-ancora). Nota livre passa por sanitizer (anti prompt-injection + termos incompativeis com estetica). Capas em `cover_library` por produtor; `variation_seeds` JSONB persiste sub_location_chosen + lighting_setup_slug + underground_camera + mood_used + artist_primario. Creditos por tier (Free 5 / Intermediario 15 / Premium 40). Upload manual nao consome creditos. Toda geracao registra em `api_usage` (incluindo cache_read/cache_write tokens). Detalhes em `docs/decisoes/2026-05-22-prompt-dna-capa-v3.md` (supersede `2026-05-21-prompt-dna-capa-v2.md`).
